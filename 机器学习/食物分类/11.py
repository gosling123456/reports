...
#                 clf = SVC(C=C[epoch // 50],kernel='rbf').fit(XX_train, Y_train)
#                 acc = clf.score(V_data, V_label)
#                 print('\n')
#                 print(model+'第%d轮，训练集上准确率：' % epoch, clf.score(XX_train, Y_train))
#                 print(model+'第%d轮，测试集上准确率：' % epoch, clf.score(XX_test, Y_test))
...
#         elif model == 'KNN':
#             for epoch in range(epochs):
#                 clf = KNeighborsClassifier(n_neighbors=K[epoch // 50]).fit(XX_train, Y_train)
#                 acc = clf.score(V_data, V_label)
#                 print('\n')
#                 print(model + '第%d轮，训练集上准确率：' % epoch, clf.score(XX_train, Y_train))
#                 print(model + '第%d轮，测试集上准确率：' % epoch, clf.score(XX_test, Y_test))
...
#
#
# def train_one_batch(images, labels, optimizer, model, epoch, batch_idx):
#     '''
...
#
#
# def evaluate_testset(test_loader, model, epoch):
#     '''
...
#     global lr_scheduler
#     print('device', device)
#     train_transform = transforms.Compose([transforms.RandomResizedCrop(256),  # 随即裁剪成224*224大小，因此要求图像超过224*224
...
#             # 交叉熵损失函数
#             criterion = nn.CrossEntropyLoss()  # 众所周知，逻辑回归的损失函数是交叉熵
#             # 训练轮次 Epoch
...
#             log_test = {}
#             log_test['epoch'] = 0
#             log_test.update(evaluate_testset(test_loader, model, epoch))
...
#                     batch_idx += 1
#                     log_train = train_one_batch(images, labels, optimizer, model, epoch, batch_idx)
...
#                 ## 测试阶段
#                 model.eval()
#                 log_test = evaluate_testset(test_loader, model, epoch)
#                 df_test_log = df_test_log.append(log_test, ignore_index=True)
#
#                 # 保存最新的最佳模型文件
...
#     X_train_path, X_test_path, XX_train, Y_train, XX_test, Y_test, V_data, V_label, di = mk_dataset(**vars(opt))
#     # print('------------------------微调--------------------------------')
#     # model_fine_tune(**vars(opt))
#     # print('------------------------训练--------------------------------')
#     model_training()
#     # model_test('TraditionModels/tree')
#%%